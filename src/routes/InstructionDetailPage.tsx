...
const InstructionDetailPage = () => {
  ...
  if (isLoading) return <LoadingState label="Đang tải instruction..." />;
  if (isError || !instruction || !instructionId) return <ErrorState message="Không tìm thấy instruction" />;
  ...
  const handleUpdateInstruction = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      window.alert('Cập nhật instruction thành công');
  ...
  const handleDeleteInstruction = async () => {
    if (!instructionId) return;
    if (!window.confirm('Bạn có chắc chắn muốn xóa instruction này?')) return;
  ...
  const handleDeleteExample = async (exampleId: string) => {
    ...
    if (!window.confirm('Xóa example này?')) return;
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <StatusBadge status={instruction.status}>{instruction.status}</StatusBadge>
            <button
              type="button"
        ...
        <CardContent>
          <form onSubmit={handleUpdateInstruction} className="grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 flex justify-end">
              ...
              >
                {isSavingInstruction ? 'Đang lưu...' : 'Lưu instruction'}
              </button>
            </div>
          </form>
        </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            <div>
              <p className="text-sm font-semibold uppercase tracking-wide text-primary-600">Rules</p>
              <h2 className="text-xl font-semibold text-slate-900">Quản lý rules</h2>
            </div>
          </CardTitle>
          <button
            type="button"
        ...
        <CardContent>
          {rules && rules.length > 0 ? (
              ...
              {rules.map((rule) => (
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-slate-900">{rule.title || 'Rule chưa đặt tên'}</p>
            ...
            <EmptyState title="Chưa có rule" description="Thêm rule mới để mô tả yêu cầu chi tiết." />
          )}
          ...
          {ruleDraft ? (
            <form onSubmit={handleSaveRule} className="mt-6 rounded-2xl border border-primary-200 bg-primary-50 px-5 py-5 space-y-3">
              ...
              <div>
                <label className="text-sm font-medium text-slate-700">Định dạng output</label>
                <textarea
                  rows={3}
              ...
              <div className="flex justify-end gap-3">
                ...
                >
                  {isSavingRule ? 'Đang lưu...' : 'Lưu rule'}
                </button>
              </div>
            </form>
          ) : null}
      ...
      {selectedRuleId ? (
        <Card>
          <CardHeader>
            <CardTitle>
              <div>
                <p className="text-sm font-semibold uppercase tracking-wide text-primary-600">Examples</p>
                <h2 className="text-xl font-semibold text-slate-900">Ví dụ cho rule</h2>
              </div>
            </CardTitle>
            <button
              type="button"
                ...
                {examples.map((example) => (
                  <div key={example.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">{example.name || 'Example'}</p>
                        <p className="text-xs text-slate-500">{example.is_gold ? 'Gold' : 'Reference'}</p>
                      </div>
